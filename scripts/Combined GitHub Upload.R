...
codelist <- read_csv(here("C:/yourpath/updatecodelist.csv"),
...
# Load ETR data
## Edit file path for local file location
df_etr <- readr::read_csv(here::here("C:/yourpath/etr_tidy.csv")) %>%
...
df_etr_historic <- readr::read_csv(here::here("C:/yourpath/etr.csv"), 
                                   col_names = FALSE) %>%
...
## Edit file path for local file location
stp_to_region_map <- read_csv(here::here("C:/yourpath/gp-reg-pat-prac-map.csv")) %>%
...
## See data_update script

df_scmd <- read_csv(here("C:/yourpath/df_complete_scmd.csv"),
...
df_dmd_info <- read_csv(here("C:/yourpath/UpdateDMDInfo.csv"),
                        col_types = cols(vmp_snomed_code = col_character(),
...
df_scmd_ddd_combi <- df_scmd_ddd %>% 
  left_join(stp_to_region_map, by = "stp_code") %>% 
  ## filter(vtmnm == c("Sodium Oxybate")) (for example) for individual drug plots
  filter(vtmnm != c("Solriamfetol"))

## Combined national prescribing  
...
temp_ggplot_comReg <- df_scmd_combi %>% 
...
# temp_ggplot
plotly::ggplotly(temp_ggplot_comReg,
                 tooltip = "text") %>%
  plotly::config(displayModeBar = FALSE)

### Volumes per STP

df_scmd_ddd_comSTP <- df_scmd_combi %>% 
...
temp_ggplot_comSTP <- df_scmd_ddd_comSTP %>% 
  filter(rank <= 15) %>% 
  ggplot(aes(stp_name)) +
  geom_bar(aes(y = volume_ddd,
               fill = vtmnm,
...
# temp_ggplot
plotly::ggplotly(temp_ggplot_comSTP,
                 tooltip = "text") %>%
  plotly::config(displayModeBar = FALSE)

#######

### Plot DDD volumes as %total rather than absolute volume per region

# Convert total_volume column to numeric
total_volume <- df_scmd_combi %>%
  summarise(total_volume = sum(volume_ddd, na.rm = TRUE)) %>%
  pull(total_volume)

# Calculate proportion for each stp_name
df_scmd_ddd_perSTP <- df_scmd_combi %>%
  # filter(year_month >= as.Date("2019-08-01") & year_month <= as.Date("2020-07-01")) %>%
  select(year_month, ods_code, vtmnm, volume_ddd, stp_name) %>%
  mutate(stp_name = fct_explicit_na(stp_name)) %>%
  group_by(stp_name, vtmnm) %>%
  ...
  group_by(stp_name) %>%
  mutate(
    prop_use = (volume_ddd / total_volume) * 100,
    pos = cumsum(volume_ddd) - volume_ddd / 2,
    total = total_volume
  ) %>%
  ungroup() %>%
  mutate(
    ...
    stp_name = fct_reorder(stp_name, prop_use, .desc = TRUE)
  )

temp_ggplot_perSTP <- df_scmd_ddd_perSTP %>%
  group_by(stp_name) %>%
  summarise(total_prop = sum(prop_use, na.rm = TRUE)) %>%
  top_n(15, total_prop) %>%
  inner_join(df_scmd_ddd_perSTP, by = "stp_name") %>%
  arrange(prop_use) %>%
  ggplot(aes(fct_reorder(stp_name, prop_use))) +
  geom_bar(
    aes(
      y = prop_use,
      fill = vtmnm,
...
plotly::ggplotly(temp_ggplot_perSTP, tooltip = "text") %>%
  plotly::config(displayModeBar = FALSE)
...
p4 <- temp_ggplot_comReg
p5 <- temp_ggplot_comSTP


(p4 + theme(legend.position = "none"))/p5
