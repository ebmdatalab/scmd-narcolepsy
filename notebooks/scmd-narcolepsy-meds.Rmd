---
title: "Exploring trends and variation in narcolepsy treatments"
...
We have prepared the following notebook for investigating the use of narcolepsy treatments in English hospitals. 
...
The file [`data/narcolepsy_meds.csv`](https://github.com/ebmdatalab/scmd-narcolepsy/blob/main/data/narcolepsy-meds.csv) contains the SNOMED codes for the medicines we are investigating, see Table below with raw data.
More information about each SNOMED code included in this analysis is shown in the methods section below (see dm+d Table).
See the document [`codelists/create-codelists.Rmd`](https://nbviewer.org/github/ebmdatalab/scmd-narcolepsy/blob/main/codelists/create-codelist-r.html) for more information about how the codelist was created.
...
The file `scipts/get_data.R` describes the pre-processing of the "NHS Trusts" data (see below)
...
df_scmd_ddd <-
  ...
  # mutate(volume_singles = case_when(form_descr == "Oral solution" ~ 1)) %>% 
...
df_scmd_ddd %>%
  filter(vmp_snomed_code != "28897511000001100") %>%
  # filter(vtmnm == "Sodium oxybate") %>%
  select(year_month, ods_code, vmp_snomed_code,
         # vmp_product_name,
         total_quantity,
         # vtmid,
         vtmnm,
         # form_cd,
         form_descr, df_ind_cd, df_descr,
         udfs, udfs_descr, strnt_nmrtr_val, strnt_nmrtr_uom, strnt_dnmtr_val, strnt_dnmtr_descr,
         ddd, volume_singles, volume_mg_strength, volume_ddd) 

# %>%
  # mutate(volume_ddd = case_when(form_descr == "Oral solution" ~ (total_quantity * strnt_nmrtr_val) / ddd,
                                # TRUE ~ volume_ddd))
...
```{r, fig.width=7.2, fig.height=3, cache=TRUE, fig.cap="Figure. National prescribing of narcolepsy treatments over time."}
...
```{r, fig.width=8, fig.height=12, fig.cap="Figure. Regional prescribing of narcolepsy treatments over time."}
...
```{r fig.width=8, fig.height=4, fig.cap="Figure. Total regional prescribing of narcolepsy treatments."}
temp_ggplot <- df_scmd_ddd_map %>% 
...
```{r, fig.width=8, fig.height=6, fig.cap="Figure. Total prescribing of narcolepsy treatments for 20 STPs with the largest volume across all selected medications."}
